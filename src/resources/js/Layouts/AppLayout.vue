...
const navigation = computed(() => {
  ...
  const items = [
    ...
    { name: 'Penugasan Guru', href: '/teaching-assignments', icon: 'teaching-assignments', show: permissions.manage_assignments },
    ...
    { name: 'Pengumuman', href: '/announcements', icon: 'announcements', show: true },
    { type: 'divider', name: 'ANALITIK', show: permissions.manage_analytics || permissions.view_own_class_analytics },
    { name: 'Analitik', href: '/analytics', icon: 'analytics', show: permissions.manage_analytics || permissions.view_own_class_analytics },
    { name: 'Laporan', href: '/analytics/reports', icon: 'report', show: permissions.manage_analytics || permissions.view_own_class_analytics },
    ...
    >
      ...
      <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <template v-for="item in navigation" :key="item.name">
          <!-- Section divider (non-clickable label) -->
          <div
            v-if="item.type === 'divider'"
            v-show="sidebarOpen"
            class="mt-5 mb-1 select-none px-3 pt-2 text-[10px] font-semibold uppercase tracking-widest text-slate-400 border-t border-slate-100/80 pt-3"
          >
            {{ item.name }}
          </div>
          ...
          <svg v-else-if="item.icon === 'teaching-assignments'" class="h-5 w-5 shrink-0" :class="isActive(item.href) ? 'text-orange-600' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
          ...
          <svg v-else-if="item.icon === 'analytics'" class="h-5 w-5 shrink-0" :class="isActive(item.href) ? 'text-orange-600' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 8v8m-4-5v5m-4-2v2M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
          <svg v-else-if="item.icon === 'report'" class="h-5 w-5 shrink-0" :class="isActive(item.href) ? 'text-orange-600' : 'text-slate-400 group-hover:text-slate-600'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
        ...
        <aside class="fixed inset-y-0 left-0 w-64 bg-white shadow-2xl">
          ...
          <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-1">
            ...
            <template v-for="item in navigation" :key="item.name">
              ...
              <div
                v-if="item.type === 'divider'"
                class="mt-5 mb-1 select-none px-3 pt-3 text-[10px] font-semibold uppercase tracking-widest text-slate-400 border-t border-slate-100/80"
              >
                {{ item.name }}
              </div>
              ...
              <svg v-else-if="item.icon === 'teaching-assignments'" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/></svg>
              ...
              <svg v-else-if="item.icon === 'analytics'" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M16 8v8m-4-5v5m-4-2v2M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
              <svg v-else-if="item.icon === 'report'" class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
